...
def _should_poll(engine) -> bool:
    """Decide whether ``engine`` is worth polling on this tick."""
    if engine.current_stage != 6:
        return False
    if engine.phase not in _ACTIVE_PHASES:
        return False
    if engine.phase == "done":
        # Skip if the stage-6 commit is older than the stale window —
        # finished projects don't need fresh polls, the run-id map
        # already shows the terminal state.
        from datetime import datetime, timezone
        started = engine.state.get("stage_started_at", {}).get("6", "")
        if not started:
            return True  # no timestamp known, poll once defensively
        try:
...
async def poll_active_projects() -> dict[str, int]:
    """Refresh ``stage_6_runs`` on every active OMC project pipeline.

    Returns a per-project map of ``{pid: n_runs}`` for telemetry. Crons
    are expected to swallow the return value but it's useful in tests.
    """
    from onemancompany.core.pipeline_engine import _active_pipelines

    active = {pid: eng for pid, eng in _active_pipelines.items() if _should_poll(eng)}
    if not active:
        return {}

    all_runs = _list_infra_runs()
    if not all_runs:
        return {pid: 0 for pid in active}

    counts: dict[str, int] = {}
    for pid, engine in active.items():
        iter_id = engine._iteration_id()
        ...
        if engine.state.get("stage_6_runs") != updated:
            engine.state["stage_6_runs"] = updated
            engine._save()
            logger.debug(
